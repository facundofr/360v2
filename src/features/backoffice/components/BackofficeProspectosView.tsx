...
const ESTADOS_DISPONIBLES = [
  "Lead", "1º Contacto", "WhatsApp enviado", "Llamada telefónica",
  "Conversación iniciada por WhatsApp", "Promoción aplicada",
  "Calificado Cotización", "Calificado Póliza", "Calificado Pago",
  "Póliza iniciada", "Póliza generada", "Póliza enviada a supervisor",
  "Póliza pendiente a firma", "Póliza firmada",
  "Venta", "Fuera de zona", "Fuera de edad",
...
export function BackofficeProspectosView() {
  ...
  const columnsProspectos = useMemo<ColumnDef<Prospecto>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const p = row.original
        return p.vendedor_nombre ? <>{p.vendedor_nombre} {p.vendedor_apellido}<br /><span className="text-xs text-muted-foreground">{p.vendedor_email}</span></> : <Badge variant="warn">Sin asignar</Badge>
      },
    },
    {
      id: "supervisor",
      header: "Supervisor",
      meta: { className: "hidden lg:table-cell text-sm" },
      accessorFn: (p) => p.supervisor_nombre ?? "",
      cell: ({ row }) => {
        const p = row.original
        return p.supervisor_nombre ? `${p.supervisor_nombre} ${p.supervisor_apellido ?? ""}` : <Badge variant="secondary">N/A</Badge>
      },
    },
    {
      ...
      cell: ({ row }) => fmtFecha(row.original.fecha_registro),
    },
    {
      id: "actividad",
      header: "Actividad",
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ], [opcionesFiltros.vendedores.length])
