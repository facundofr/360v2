...
export function BackofficeSupervisoresView() {
  ...
  const columnsSupervisores = useMemo<ColumnDef<Supervisor>[]>(() => [
    { id: "nombre", header: "Supervisor", accessorFn: getNombre, cell: ({ row }) => <span className="font-medium text-sm">{getNombre(row.original)}</span> },
    {
      accessorKey: "email",
      header: "Email",
    ...
    },
    {
      ...
      header: "Vendedores",
      meta: { className: "text-center font-semibold", headerClassName: "text-center" },
      cell: ({ row }) => row.original.total_vendedores ?? "—",
    },
    {
      accessorKey: "total_prospectos",
      header: "Prospectos",
      meta: { className: "text-center hidden md:table-cell", headerClassName: "text-center hidden md:table-cell" },
      cell: ({ row }) => row.original.total_prospectos ?? "—",
    },
    {
      accessorKey: "ventas",
      header: "Ventas",
      meta: { className: "text-center hidden md:table-cell font-semibold", headerClassName: "text-center hidden md:table-cell" },
      cell: ({ row }) => row.original.ventas ?? "—",
    },
    {
      accessorKey: "is_enabled",
      header: "Estado",
      cell: ({ row }) => (
        <Badge variant={row.original.is_enabled !== false ? "ok" : "secondary"}>
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ], [vendedoresSin.length, savingToggle])

  const columnsVendedoresEquipo = useMemo<ColumnDef<VendedorEquipo>[]>(() => [
    {
      id: "nombre",
      header: "Nombre",
      ...
      cell: ({ row }) => (
        <>
          <p className="text-sm font-medium">{row.original.first_name} {row.original.last_name}</p>
          {row.original.categoria_nombre && <p className="text-xs text-muted-foreground">{row.original.categoria_nombre}</p>}
        </>
      ),
    },
    {
    ...
    {
      accessorKey: "prospectos",
      header: "Prospectos",
      meta: { className: "text-right", headerClassName: "text-right" },
      cell: ({ row }) => <Badge variant="secondary">{row.original.prospectos ?? "—"}</Badge>,
    },
    {
      accessorKey: "ventas",
      header: "Ventas",
      meta: { className: "text-right", headerClassName: "text-right" },
      cell: ({ row }) => <Badge variant="ok">{row.original.ventas ?? "—"}</Badge>,
    },
    {
      accessorKey: "is_enabled",
      header: "Estado",
      cell: ({ row }) => (
        <Badge variant={row.original.is_enabled !== false ? "ok" : "secondary"} className="text-xs">
          {row.original.is_enabled !== false ? "Activo" : "Inactivo"}
        </Badge>
      ),
    },
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                {(detalle.vendedores?.length ?? 0) === 0 ? (
                  ...
                  <DataTable columns={columnsVendedoresEquipo} data={detalle.vendedores ?? []} hideColumnToggle />
                )}
              </div>
            </div>
