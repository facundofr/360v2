...
const ESTADOS_POLIZA = ["pendiente", "vigente", "cancelada", "suspendida", "vencida"]
...
export function BackofficePolizasView() {
  ...
  const guardarEstado = async () => {
    ...
    try {
      await axios.patch(`${API_URL}/backoffice/polizas/${polizaSeleccionada.id}/estado`, { estado: nuevoEstado, motivo: motivoCambio }, { headers: getAuth() })
          ...
          {loading ? <Skeleton className="h-64 w-full" /> : polizas.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-20">Póliza</TableHead>
          ...
          <div className="space-y-4 pt-2">
            <div>
              ...
              <Select value={nuevoEstado} onValueChange={setNuevoEstado}>
                ...
                <SelectContent>
                  {ESTADOS_POLIZA.map(e => <SelectItem key={e} value={e}>{e}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div>
