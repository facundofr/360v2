...
const initialForm: Promocion = {
  id: null, nombre: "", descripcion: "", descuento_porcentaje: 0, activa: true
}
...
export function BackofficePromocionesView() {
  ...
  const abrirEditar = (p: Promocion) => {
    setFormData({ id: p.id, nombre: p.nombre, descripcion: p.descripcion ?? "", descuento_porcentaje: p.descuento_porcentaje ?? 0, activa: p.activa ?? true })
  ...
  const guardar = async () => {
    ...
    setSaving(true)
    try {
      const payload = { nombre: formData.nombre?.trim(), descripcion: formData.descripcion?.trim(), descuento_porcentaje: Number(formData.descuento_porcentaje), activa: formData.activa }
      if (formData.id) {
                ...
                <TableBody>
                  {filtradas.length === 0 ? (
                    ...
                    <TableRow key={p.id}>
                      ...
                      <TableCell className="hidden md:table-cell text-xs text-muted-foreground">{fmtFecha(p.created_at)}</TableCell>
                      <TableCell>
      ...
      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent className="sm:max-w-lg lg:max-w-xl" showCloseButton={false}>
          ...
          <div className="flex flex-col gap-4 pt-2">
            ...
            <div>
              <Label className="text-sm mb-1.5 block">Descuento (%)</Label>
