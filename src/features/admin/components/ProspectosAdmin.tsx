...
export default function ProspectosAdmin() {
  ...
  const cambiarEstado = async () => {
    ...
    try {
      await axios.patch(`${API_URL}/admin/prospectos/${estadoModal.prospecto.id}/estado`, { estado: nuevoEstado }, { headers: getHeaders() })
    ...
    <div className="flex gap-1 flex-wrap">
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem onClick={() => { setEstadoModal({ open: true, prospecto: p }); setNuevoEstado(p.estado ?? "") }}>
  ...
  return (
    <div className="space-y-4">
          ...
          {vista === "tabla" && (
            <div className="rounded-lg border overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="min-w-[150px]">Prospecto</TableHead>
                    <TableHead className="min-w-[130px]">Vendedor</TableHead>
                    ...
                    <TableHead className="w-24 text-center">Reciclado</TableHead>
                    <TableHead className="min-w-[140px]">Fecha</TableHead>
                    <TableHead className="min-w-[190px]">Acciones</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {paginaActual.map(p => (
                    <TableRow key={p.id}>
                      <TableCell>
                        <div className="font-semibold text-sm leading-tight">{p.nombre} {p.apellido}</div>
                      ...
                      <TableCell>
                        <div className="space-y-0.5 text-xs">
                          <div className="flex items-center gap-1 text-muted-foreground">
                            ...
                            <span className="truncate max-w-[110px]">{p.telefono ?? "-"}</span>
                          </div>
                          <div className="flex items-center gap-1 text-muted-foreground">
                            <Mail className="size-3 shrink-0" />
                            <span className="truncate max-w-[110px]">{p.email ?? "-"}</span>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>{getBadgeEstado(p.estado ?? "")}</TableCell>
                      <TableCell className="text-center">
                        <span className="text-xs bg-muted rounded px-1.5 py-0.5">{p.edad ?? "-"}</span>
                      </TableCell>
                      <TableCell className="text-center">
                        {p.es_reciclado ? (
          ...
          {detalleModal.prospecto && (
            <div className="space-y-3">
              ...
              <div className="flex gap-2 flex-wrap pt-1">
                <Button size="sm" variant="outline" onClick={() => { setEstadoModal({ open: true, prospecto: detalleModal.prospecto }); setNuevoEstado(detalleModal.prospecto?.estado ?? ""); setDetalleModal(s => ({ ...s, open: false })) }}>
      ...
      <Dialog open={estadoModal.open} onOpenChange={open => setEstadoModal({ open, prospecto: open ? estadoModal.prospecto : null })}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
