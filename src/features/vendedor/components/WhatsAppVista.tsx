...
interface Conversacion {
  ...
  /** Alias real: `c.ultima_actividad` (`chatController.js`/`chatService.js`), no `updated_at`. */
  ultima_actividad?: string
  /** activa | pausada | cerrada — validado así en `chatController.js` (vendedor). */
  estado?: string
}
...
/** Forma real de `GET /chat/estadisticas` (`chatController.js`, vendedor) —
 * no existen `mensajes_hoy`/`sin_responder`. */
interface EstadisticasChat {
  total_conversaciones?: number
  conversaciones_activas?: number
  conversaciones_pausadas?: number
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-muted-foreground shrink-0 ml-1">{formatHora(conv.ultima_actividad)}</span>
                  </div>
        ...
        {!conversacionActual ? (
          ...
          <>
            ...
            <div className="flex items-center gap-3 px-4 py-3 border-b bg-muted/30">
              ...
              <Select
                value={conversacionActual.estado ?? "activa"}
            ...
            {/* Conversación cerrada: sin compositor, igual que producción
                (`WhatsAppVista.jsx:991`, oculta el input entero). */}
