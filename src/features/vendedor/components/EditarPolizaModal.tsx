...
const PASOS = ["Datos Personales","Declaración Jurada","Integrantes","Referencias","Salud y Términos"]
...
interface Integrante { nombre: string; apellido: string; dni?: string; vinculo: string; fecha_nacimiento?: string; sexo?: string }
...
export function EditarPolizaModal({ polizaId, open, onClose, onActualizada }: EditarPolizaModalProps) {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_URL}/polizas/vendedor/${polizaId}/editar`, { headers: authHeaders })
      .then(r => {
  ...
  const guardar = async () => {
    ...
    try {
      ...
      await axios.put(`${API_URL}/polizas/vendedor/${polizaId}/actualizar`, payload, { headers: authHeaders })
