...
// Valores en minúscula a propósito: así los guarda producción en la base
// (`PasoDatosPersonales.jsx:911-913`) — cambiar la capitalización rompería el
// filtrado/comparación contra pólizas ya creadas en prod.
export const SEXOS = ["masculino", "femenino", "otro"] as const

export const VINCULOS = ["pareja/conyuge", "hijo/a", "familiar a cargo"] as const

// Lista real de producción (`PasoDatosPersonales.jsx:606-610`) — no confundir
// con la lista de coberturas médicas anteriores (`COBERTURAS_MEDICAS` más abajo).
export const OBRAS_SOCIALES = ["OSDEPYM", "OSTVLA", "OSFE", "Otra"] as const
...
/**
 * El bloque de datos de empresa se OCULTA sólo para "Particular/autónomo"
 * (`esParticular` en `PasoDatosPersonales.jsx:27-32` y
 * `PasoIntegrantesDocumentos.jsx:210-215`) — se muestra tanto para "Con recibo
 * de sueldo" como para "Monotributista". No usar `TIPO_AFILIACION_CON_RECIBO`
 * para esta condición, sólo sirve para el requerido de recibo de sueldo.
 */
export const TIPO_AFILIACION_PARTICULAR = 1
