...
// Antes tenía 15 valores inventados/sin tilde (ej. "Calificado Cotizacion")
// contra el whitelist real de 27 estados de `supervisorController.js` —
// el cambio de estado devolvía 400 para 6 de ellos y faltaban 12 más.
...
type VistaType = "dashboard" | "prospectos" | "vendedores" | "polizas" | "documentos" | "promociones" | "metricas" | "resumen" | "cotizaciones"
...
export default function SupervisorDashboardPage({ vistaInicial = "dashboard" }: SupervisorDashboardPageProps = {}) {
  ...
  const cambiarEstado = async () => {
    ...
    try {
      await axios.patch(
        ...
        { estado: nuevoEstado, motivo: comentarioEstado },
        { headers: authHeaders }
      )
  ...
  // Muestra el historial real de la conversación (GET
  // /supervisor/chat/conversaciones/prospecto/:id), no un wa.me a ciegas: el
  // supervisor necesita ver qué se habló antes de intervenir.
  ...
  const abrirWhatsApp = (p: Prospecto) => {
    ...
    if (p.numero_contacto) {
      setConversacionModal({ open: true, prospecto: p })
    } else {
  ...
  const columnsProspectos = useMemo<ColumnDef<Prospecto>[]>(() => [
    {
      ...
      meta: { className: "w-14" },
      accessorFn: (p) => p.id,
      cell: ({ row }) => <Badge variant="secondary" className="text-xs font-bold">{row.original.id}</Badge>,
    },
    {
      id: "nombre",
      header: "Nombre",
    ...
    },
    {
      ...
      accessorFn: (p) => p.edad ?? 0,
      cell: ({ row }) => row.original.edad ? <Badge variant="secondary">{row.original.edad} años</Badge> : "-",
    },
    {
      id: "estado",
      header: "Estado",
    ...
    },
    {
      ...
      accessorFn: (p) => p.vendedor_nombre ?? "-",
    },
    {
      id: "acciones",
      header: "Acciones",
      ...
      <Dialog open={modalEstado.open} onOpenChange={(open: boolean) => setModalEstado({ open, prospecto: open ? modalEstado.prospecto : null })}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={cambiarEstado} disabled={!nuevoEstado || !comentarioEstado.trim() || guardandoEstado}>
