...
import {
  ...
  Percent, LayoutGrid, List, ExternalLink, Loader2, FilePlus
} from "lucide-react"
  ...
  const guardarEstado = async () => {
    ...
    try {
      await axios.patch(
        ...
        { estado: nuevoEstado, motivo: motivoCambio },
        { headers: getAuth() }
      )
                    ...
                    <TableBody>
                      {polizas.map(pol => (
                        <TableRow key={pol.id}>
                          ...
                          <TableCell>{getBadgePolizaEstado(pol.estado)}</TableCell>
                          <TableCell className="hidden sm:table-cell text-sm">{fmtPeso(pol.total_final)}</TableCell>
                          <TableCell className="hidden sm:table-cell text-xs text-muted-foreground">{fmtFecha(pol.created_at)}</TableCell>
                          <TableCell>
      ...
      <Dialog open={cambioEstadoModal} onOpenChange={setCambioEstadoModal}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={guardarEstado} disabled={savingEstado || !nuevoEstado}>
