...
import {
  ...
  Trash2, Tag, Search, RefreshCw, ChevronUp, ChevronDown,
  ChevronsUpDown, Loader2, BarChart2
} from "lucide-react"
...
type SortField = "id" | "nombre" | "apellido" | "email" | "estado"
...
export function SupervisorVendedoresView() {
  ...
  const vendedoresFiltrados = vendedores
    ...
    .sort((a, b) => {
      ...
      if (sortField === "nombre") { va = a.first_name?.toLowerCase() ?? ""; vb = b.first_name?.toLowerCase() ?? "" }
      else if (sortField === "apellido") { va = a.last_name?.toLowerCase() ?? ""; vb = b.last_name?.toLowerCase() ?? "" }
      else if (sortField === "email") { va = a.email?.toLowerCase() ?? ""; vb = b.email?.toLowerCase() ?? "" }
      else if (sortField === "estado") { va = isEnabled(a) ? 1 : 0; vb = isEnabled(b) ? 1 : 0 }
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="cursor-pointer select-none" onClick={() => handleSort("nombre")}>
                    <span className="flex items-center gap-1">Nombre <SortIcon field="nombre" sortField={sortField} sortDir={sortDir} /></span>
                  </TableHead>
                  <TableHead className="cursor-pointer select-none" onClick={() => handleSort("apellido")}>
                    <span className="flex items-center gap-1">Apellido <SortIcon field="apellido" sortField={sortField} sortDir={sortDir} /></span>
                  </TableHead>
                  ...
                  <TableHead className="cursor-pointer select-none" onClick={() => handleSort("estado")}>
                    <span className="flex items-center gap-1">Estado <SortIcon field="estado" sortField={sortField} sortDir={sortDir} /></span>
                  </TableHead>
              ...
              <TableBody>
                {vendedoresFiltrados.length === 0 ? (
                  ...
                  <TableRow key={v.id} className={!isEnabled(v) ? "opacity-60" : undefined}>
                    <TableCell className="font-bold text-primary text-sm">{v.id}</TableCell>
                    <TableCell className="text-sm">{v.first_name}</TableCell>
                    <TableCell className="text-sm">{v.last_name}</TableCell>
                    <TableCell className="text-sm max-w-[200px] truncate">{v.email}</TableCell>
                    <TableCell className="text-sm">{v.phone_number || "—"}</TableCell>
                    <TableCell>{categoriaBadge(v)}</TableCell>
                    <TableCell>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="hidden lg:table-cell">F. Asignación</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {prospectosVendedor.map(p => (
                        <TableRow key={p.id}>
                          <TableCell>
                            <input
                              type="checkbox"
                              ...
                              onChange={() => toggleProspecto(p.id)}
                            />
                          </TableCell>
                          <TableCell className="font-medium text-sm">
                            {p.nombre} {p.apellido}
                            <div className="md:hidden text-xs text-muted-foreground">{p.numero_contacto}</div>
                          </TableCell>
                          <TableCell className="hidden md:table-cell text-xs text-muted-foreground">
                            <div>{p.numero_contacto}</div>
                            <div>{p.correo}</div>
                          </TableCell>
                          <TableCell>{getBadgeEstado(p.asignacion_estado ?? p.estado ?? "")}</TableCell>
